...
public class Trainer {
	...
	private FeedforwardNeuralNetwork bestGen;
...
//		int outputDim = Commons.BREAKOUT_NUM_ACTIONS;
		...
		neuralNetwork = new FeedforwardNeuralNetwork(hiddenDim);
		...
		bestGen = new FeedforwardNeuralNetwork(hiddenDim);
	}
	...
	public void train(int numGenerations) {
		for (int generation = 0; generation < numGenerations; generation++) {
			...
			if (geneticAlgorithm.getBestFitnessPop() > bestOverallFitness) {
				...
				bestOverallFitness = geneticAlgorithm.getBestFitnessPop(); // Atualizar o melhor fitness geral
			}
		}
		System.out.println("Best Overall Fitness: " + bestOverallFitness);
	}

	public FeedforwardNeuralNetwork getTrainedNeuralNetwork() {
		return neuralNetwork;
	}

	public FeedforwardNeuralNetwork getBestGen() {
		return bestGen;
	}

	// Método para clonar uma rede neural
	private FeedforwardNeuralNetwork cloneNeuralNetwork(FeedforwardNeuralNetwork originalNetwork) {
		FeedforwardNeuralNetwork clonedNetwork = new FeedforwardNeuralNetwork(originalNetwork.getHiddenDim());
	...
	FeedforwardNeuralNetwork getNN() {
		return neuralNetwork;
	}
	...
	public static void main(String[] args) {
		...
		FeedforwardNeuralNetwork bestGen = trainer.cloneNeuralNetwork(trainer.getTrainedNeuralNetwork());
		...
		// comparação
		FeedforwardNeuralNetwork feed = new FeedforwardNeuralNetwork(30);

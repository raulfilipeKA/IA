...
public class FeedforwardNeuralNetwork implements GameController {
	// Constantes definindo o tamanho da entrada e saída
	...
	private int hiddenDim; // Dimensão da camada oculta configurável
	...
	private void initializeParameters() {
		...
		// Inicializar pesos e biases da camada de saída
		for (int i = 0; i < hiddenDim; i++) {
	...
	public int nextMove(int[] currentState) {
		...
		// Propagação da entrada até a camada oculta
		for (int i = 0; i < hiddenDim; i++) {
		...
		// Propagação da camada oculta até a saída
		for (int i = 0; i < OUTPUT_DIM; i++) {
		...
		// Decisão de qual ação tomar baseada na saída da rede
	...
	/*
	 * private int chooseAction(double[] output) { // Escolha a ação baseada na
	 * saída da rede double probability = sigmoid(output[0]); // Aplica sigmoid à
	 * primeira saída if (probability > 0.5) { return 1; // Escolhe a ação 1 se a
	 * probabilidade for maior que 0.5 } else { return 2; // Caso contrário, escolhe
	 * a ação 2 } }
	 */
	...
	public void updateParameters(double[] newWeights) {
		...
		// Atualizar biases da camada de saída
		for (int i = 0; i < OUTPUT_DIM; i++) {
	...
	public double[] getParameters() {
		...
		// Extrair biases da camada de saída
		for (int i = 0; i < OUTPUT_DIM; i++) {
